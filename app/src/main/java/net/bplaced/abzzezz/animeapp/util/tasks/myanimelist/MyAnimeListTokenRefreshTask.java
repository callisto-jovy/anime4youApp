...
public class MyAnimeListTokenRefreshTask extends TaskExecutor implements Callable<Optional<MyAnimeListToken>>, MyAnimeListHolder {

    private final MyAnimeListToken token;

    public MyAnimeListTokenRefreshTask(final MyAnimeListToken token) {
        this.token = token;
    }
    ...
    public Optional<MyAnimeListToken> call() {
        ...
        args.put(GRANT_TYPE_TOKEN_REFRESH[0], GRANT_TYPE_TOKEN_REFRESH[1]);
        args.put(CLIENT_ID[0], CLIENT_ID[1]);
        args.put("refresh_token", token.getRefreshToken());

        final Document document;
        try {
            document = Jsoup
                    .connect(MAL_API_TOKEN_REFRESH)
